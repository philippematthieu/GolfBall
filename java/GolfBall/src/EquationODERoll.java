 ...
 */
	public double[] getEvaluation(double s, double q[] ){
		...
		dQ[0] = vx;
		dQ[1] =  -(5.0/7.0)*getRhoGreen*getG*vx/Math.sqrt(vx*vx+vy*vy+vz*vz);			// vx longueur
		dQ[2] = vy;
		dQ[3] = -(5.0/7.0)*getRhoGreen*getG*vy/Math.sqrt(vx*vx+vy*vy+vz*vz); 			// vy largeur
		dQ[4] = 0.0;
		dQ[5] = 0.0; 	// vz hauteur nule

...
Ball::Ball(string pMarque, int pNbAlveoles, int pNbPieces, Club pGolfClub, double pTimeMax, double pdt):
					marque(pMarque),	nbAlveoles(pNbAlveoles),	nbPieces(pNbPieces),	temperature(pGolfClub.getTemperature()),	masse(0.04545),	rayon(0.0215),	area(M_PI*rayon*rayon),	rhoAir(1.292*273.15/(temperature + 273)),	rhoGreen(0.131),	g(9.81),	timeMax(pTimeMax),	dt(pdt),	alphaClubPath(pGolfClub.getAlphaClubPathRadian())
{
	...
	double v0Ballbfnms =  pGolfClub.getClubV0ms() * cos(pGolfClub.getDynamiqueLoft())*(1.0 + pGolfClub.getEcoeff())/(1+masse/pGolfClub.getPoids()) ;// Vitesse longitudinale dans le referentiel de decollage apres impact (ref: The physics of golf: The optimum loft of a driverA. Raymond Penner)
	double v0Ballbfpms = -pGolfClub.getClubV0ms() * sin(pGolfClub.getDynamiqueLoft())/(1.0 + masse/pGolfClub.getPoids() + 2.5);     // Vitesse perpendiculaire dans le referentiel de decollage apres impact


	// Calcul de l'angle de decollage de la balle
	launchAngle		= pGolfClub.getDynamiqueLoft() + atan(v0Ballbfpms/v0Ballbfnms);    // angle de decollage de la balle
	...
	v0Currentms = new double(v0Initms); // copie des valeurs initiales v0Initms
	...
	paramEqn[1] = v0Initms[1] * sin(pGolfClub.getDynamiqueLoft())    * pGolfClub.getCoeffBackSpin() / 9.54929659643;	// wy Spin dans l'axe largeur backspin
...
double* Ball::getV0Initms() {
	return v0Initms;
}
double* Ball::getVCurrentms() {
	return v0Currentms;
}
